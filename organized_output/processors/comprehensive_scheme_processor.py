#!/usr/bin/env python3
"""
Comprehensive Scheme Processor - Extracts ALL data from JSON
"""
...
def extract_scheme_data(json_data: Dict) -> Dict[str, Any]:
    """Extract and structure scheme data from JSON - COMPREHENSIVE VERSION"""
    scheme = {}
    ...
    if scheme_definitions:
        scheme['schemeDefinitions'] = scheme_definitions
...
def get_comprehensive_template() -> str:
    """Get the comprehensive Jinja2 template"""
    return """{# ====== HEADER ====== #}
# {{ scheme.schemeName }}

{% if scheme.schemeShortTitle and scheme.schemeShortTitle != scheme.schemeName %}
...
{% if scheme.schemeOpenDate %}
**Launch Date:** {{ scheme.schemeOpenDate }}
{% endif %}

{% if scheme.schemeCloseDate %}
...
{% if scheme.implementingAgency or scheme.nodalDepartment %}
**Contacts & Authorities:** 
{% if scheme.implementingAgency %}Implementing agency: {{ scheme.implementingAgency }}.{% endif %}
{% if scheme.nodalDepartment %}Nodal Department: {{ scheme.nodalDepartment }}.{% endif %}
{% endif %}

{# ====== SCHEME DEFINITIONS ====== #}
{% if scheme.schemeDefinitions %}
**Scheme Definitions:** 
{% for def in scheme.schemeDefinitions %}
...
def generate_description_with_jinja2(scheme: Dict[str, Any]) -> str:
    ...
    template = Template(get_comprehensive_template())
...
def generate_description_simple(scheme: Dict[str, Any]) -> str:
    ...
    if scheme.get('schemeDefinitions'):
        template_parts.append(f"\n**Scheme Definitions:**")
        for def_item in scheme['schemeDefinitions']:
            template_parts.append(f"- **{def_item.get('name', '')}:** {def_item.get('definition', '')}")
...
def process_single_scheme(input_file: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        metadata = {
            ...
            'template_used': 'comprehensive_template',
        ...
        result['success'] = True
        print(f"✓ Processed: {result['scheme_name']} ({len(description.split())} words)")
...
def main():
    ...
    output_directory = "/Users/priyankjairaj/Downloads/ctu-flowrag/comprehensive_schemes"
    
    print("=== Comprehensive Scheme Processor ===")
    print(f"Input directory: {input_directory}")
    print(f"Output directory: {output_directory}")
    print("Processing schemes individually with COMPREHENSIVE data extraction...")
