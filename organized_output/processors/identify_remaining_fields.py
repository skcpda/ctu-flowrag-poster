#!/usr/bin/env python3
"""
Identify remaining fields we can capture to improve extraction efficiency
"""
...
def analyze_remaining_fields(json_data, current_path="", field_sizes=None, max_depth=10, depth=0):
    """Recursively analyze field sizes in JSON to find remaining opportunities"""
    ...
    if isinstance(json_data, dict):
        for key, value in json_data.items():
            ...
            if isinstance(value, str):
                ...
                analyze_remaining_fields(value, new_path, field_sizes, max_depth, depth + 1)
    
    elif isinstance(json_data, list):
        for i, item in enumerate(json_data):
            new_path = f"{current_path}[{i}]"
            if isinstance(item, (dict, list)):
                analyze_remaining_fields(item, new_path, field_sizes, max_depth, depth + 1)
...
def identify_remaining_opportunities():
    """Identify fields we're still not capturing that have significant content"""
    input_dir = Path("/Users/priyankjairaj/Downloads/MoTA/mySchemeData")
    
    all_field_sizes = defaultdict(list)
    processed_files = 0
    
    print("Analyzing remaining field opportunities...")
    ...
    for json_file in json_files:
        try:
            ...
            # Analyze field sizes
            field_sizes = analyze_remaining_fields(json_data)
    ...
    # Fields we're currently extracting (approximate list)
    ...
    print(f"\n=== TOP REMAINING FIELDS (NOT CURRENTLY EXTRACTED) ===")
    print(f"Found {len(missing_fields)} fields with significant content we're not extracting")
    print()
    
    for i, (field_path, stats) in enumerate(missing_fields[:50]):
    ...
    total_missing_words = sum(stats['total_words'] for _, stats in missing_fields)
    print(f"\n=== POTENTIAL IMPROVEMENT ===")
    print(f"Total words in missing fields: {total_missing_words:,}")
    print(f"Potential additional content per document: {total_missing_words / processed_files:.1f} words")
    print(f"Potential total improvement: {total_missing_words / (1522968 - 51765):.1%}")
    
    return missing_fields

if __name__ == "__main__":
    missing_fields = identify_remaining_opportunities()
