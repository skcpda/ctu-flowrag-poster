#!/usr/bin/env python3
"""
Mega Extraction Processor - Extract the MASSIVE missing fields we just discovered
"""
...
def extract_mega_fields_from_json(json_data: Dict) -> Dict[str, Any]:
    """Extract MEGA fields - the massive missing content we just discovered"""
    scheme = {}
        ...
        if len(app_process) > 1:
            ...
            for i, process in enumerate(app_process[1:], 1):
                ...
                if process_md:
                    ...
                    if process_structured:
                        additional_processes.append(f"**Process {i+1}:** {clean_text(extract_all_text_from_structure(process_structured))}")
...
def generate_mega_description(scheme: Dict[str, Any]) -> str:
    """Generate description with ALL extracted content - MEGA TEMPLATE"""
    template_parts = []
...
def process_single_scheme(input_file: Path, output_dir: Path) -> Dict[str, Any]:
    """Process a single scheme file with mega extraction"""
    result = {
        'success': False,
        'scheme_name': 'Unknown',
        'error': None,
    ...
    try:
        ...
        # Extract MEGA fields from scheme data
        scheme = extract_mega_fields_from_json(json_data)
        ...
        description = generate_mega_description(scheme)
...
def process_all_schemes(input_dir: str, output_dir: str):
    """Process all schemes with mega extraction"""
    ...
    print("Processing schemes with MEGA content extraction...")
    ...
    # Print final summary
    print(f"\n=== MEGA EXTRACTION PROCESSING COMPLETE ===")
...
def main():
    ...
    output_directory = "/Users/priyankjairaj/Downloads/ctu-flowrag/mega_schemes"
    
    print("=== Mega Extraction Processor ===")
